...
/// Application configuration persisted as a TOML file in the user's config
/// directory. Stores boost behavior toggles, process whitelist/blacklist, and
/// the selected game executable path.
...
impl AppConfig {
    ...
    /// Load the configuration from disk, falling back to defaults if the file
    /// does not exist or contains invalid TOML.
    pub fn load() -> Self {
    ...
    /// Serialize the current configuration to the TOML config file, creating
    /// the parent directory if it does not yet exist.
    pub fn save(&self) {
        let path = Self::config_path();
