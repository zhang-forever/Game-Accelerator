...
/// Result summary returned by [`run_boost`], containing per-step success
/// indicators and a list of any errors encountered during the boost.
...
/// Execute the full one-click boost pipeline: kill background processes, clean
/// memory, switch power plan, enable Game Mode, and boost the game process
/// priority. Each step is gated by the corresponding flag in `config`.
